...
public struct RootView: View
{
    ...
    public init(store: Store<Root.Input, Root.State>.Proxy)
    {
        self.store = store
    }
    ...
    public var body: some View
    {
        VStack {
            GeometryReader { geometry in
                GameView(
                    store: self.store.game.contramapInput(Root.Input.game),
        ...
        .navigationBarItems( // TODO: Add different configuration screen
    ...
    private func patternSelectView() -> some View
    {
        ...
        let substore = Store<PatternSelect.Input, PatternSelect.State>.Proxy(
            state: stateBinding,
            send: { self.store.send(Root.Input.patternSelect($0)) }
        )
...
struct RootView_Previews: PreviewProvider
{
    static var previews: some View
    {
        ...
        return Group {
            lifegameView.previewLayout(.sizeThatFits)
                .previewDisplayName("Portrait")

            lifegameView.previewLayout(.fixed(width: 568, height: 320))
