...
{
    private let store: Store<PatternSelect.Input, PatternSelect.State>.Proxy

    init(store: Store<PatternSelect.Input, PatternSelect.State>.Proxy)
    {
        self.store = store
    }
        ...
        HStack {
            ...
            TextField(
                ...
                text: self.store.stateBinding(
                    get: { $0.searchText },
                    onChange: PatternSelect.Input.updateSearchText
                )
            )
...
struct PatternSelectView_Previews: PreviewProvider
{
    ...
    {
        let lifegameView = PatternSelectView(
            store: .init(
                state: .constant(.init()),
                send: { _ in }
            )
